...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        super().__init__()
        self.config = config
        assert not (not config.enforce_eager and getattr(config, 'free_cache_engine', False)), \
        ...
        self.inference_engine = LLM(
            ...
            disable_log_stats=getattr(config, 'disable_log_stats', True),
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if vllm_version in ('0.3.1', '0.4.2', '0.5.4', '0.6.3') and getattr(self.config, 'free_cache_engine', False):
        ...
        # ---- meta_info overrides (align with customized vllm_rollout) ----
        ...
        with self.update_sampling_params(**kwargs):
            ...
            response = pad_2d_list_to_length(response, self.pad_token_id,
                                             max_length=kwargs.get('max_tokens',
                                                                   self.config.response_length)).to(idx.device)

            effective_n = kwargs.get('n', self.sampling_params.n)
