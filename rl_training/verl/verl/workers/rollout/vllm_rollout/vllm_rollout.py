...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, actor_module: nn.Module, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        self.inference_engine = LLM(actor_module,
                                    ...
                                    swap_space=getattr(config, 'swap_space', 0),
    ...
    def generate_sequences(self, prompts: DataProto, max_retries: int = 1e9, **kwargs) -> DataProto:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if prompts.meta_info.get('val_top_p', None) is not None:
                    kwargs['top_p'] = prompts.meta_info['val_top_p']
                if prompts.meta_info.get('val_max_tokens', None) is not None:
                    kwargs['max_tokens'] = prompts.meta_info['val_max_tokens']
                if prompts.meta_info.get('override_n', None) is not None:
                    kwargs['n'] = int(prompts.meta_info['override_n'])
                # Generate sequences
                ...
                # Pad sequences if needed. When val_max_tokens overrides the
                # generation length (e.g. entropy scoring / validation probes),
                # pad only to that effective response length instead of the full
                # training response_length to avoid unnecessary long sequences.
                ...
                if response.shape[1] < target_response_length:
                    response = pad_sequence_to_length(
                        response, target_response_length, self.pad_token_id)
                    log_probs = pad_sequence_to_length(
                        log_probs, target_response_length, self.pad_token_id)

                # Handle multiple samples per prompt (use effective n
                # which may differ from self.config.n when override_n is set)
                effective_n = kwargs.get('n', self.config.n)
                ...
                self.inference_engine = LLM(
                    ...
                    swap_space=getattr(self.config, 'swap_space', 0),
                    skip_tokenizer_init=False,
