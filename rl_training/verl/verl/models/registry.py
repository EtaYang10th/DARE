...
from transformers import LlamaConfig, MistralConfig, GemmaConfig, Qwen2Config, SmolLM3Config
...
def check_model_support_rmpad(model_type: str):
    ...
    if model_type not in _REOVEPAD_MODELS:
        raise ValueError(
            f"Model architecture {model_type} is not supported for HF remove-padding right now. "
            f"Supported architectures: {_REOVEPAD_MODELS.keys()}. "
