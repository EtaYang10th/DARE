...
def qwen2_dtensor_weight_loader(actor_weights: Dict, vllm_model: nn.Module) -> nn.Module:
    ...
    params_dict = dict(vllm_model.named_parameters(remove_duplicate=False))
    # Iterate over a snapshot of keys so we can delete from actor_weights
    for orig_name in list(actor_weights.keys()):
        loaded_weight = actor_weights[orig_name]
        name = orig_name
        if "rotary_emb.inv_freq" in name:
            del actor_weights[orig_name]
            del loaded_weight
            continue
        if vllm_model.config.tie_word_embeddings and "lm_head.weight" in name:
            del actor_weights[orig_name]
        ...
        for param_name, weight_name, shard_id in stacked_params_mapping:
            ...
            weight_loader(param, local_loaded_weight.to(dtype=param.dtype))
            del local_loaded_weight
        # Free the state_dict entry after loading to reduce peak CPU memory
        del actor_weights[orig_name]
        del loaded_weight
...
def load_dtensor_weights(actor_weights: Dict, vllm_model: nn.Module):
    ...
    # Force GC after weight loading to free any lingering CPU tensors
    import gc
    gc.collect()
