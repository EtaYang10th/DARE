...
class Worker(Worker):
    ...
    def sync_model_weights(self, actor_weights: Dict, load_format: str):
        # Restore parameter storage if it was freed by offload_model_weights
        self._restore_param_storage()
    ...
    def offload_model_weights(self) -> None:
        # Memory-efficient offload: instead of keeping a full CPU copy
        # (which costs ~7GB per worker × 8 workers = 56GB CPU RAM),
        # we free the underlying storage of each parameter while preserving
        # the tensor metadata (shape, dtype). The weights will be fully
        # re-synced from FSDP state_dict before the next generate call.
        #
        # Before sync_model_weights can work, we need to restore the storage.
        # This is handled by _restore_param_storage() called in sync_model_weights.
        if not hasattr(self, '_param_meta'):
            self._param_meta = {}
            for name, p in self.model_runner.model.named_parameters():
                self._param_meta[name] = (p.shape, p.dtype, p.device)

        for name, p in self.model_runner.model.named_parameters():
            # Resize storage to 0 bytes — frees memory but keeps tensor metadata
            p.data = torch.empty(0, dtype=p.dtype, device='cpu')

    def _restore_param_storage(self):
        """Restore parameter storage before sync_model_weights.
        
        Instead of allocating all parameters at once (~7GB peak),
        we only mark which params need restoration. The actual allocation
        happens lazily during weight loading when each param is written to.
        """
        if not hasattr(self, '_param_meta'):
            return
        for name, p in self.model_runner.model.named_parameters():
            if p.nelement() == 0 and name in self._param_meta:
                shape, dtype, _ = self._param_meta[name]
                # Allocate on CPU; will be overwritten by weight_loader then moved to CUDA
                p.data = torch.empty(shape, dtype=dtype, device='cpu')
