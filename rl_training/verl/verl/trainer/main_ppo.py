...
from rllm.rewards.math_reward import rllm_reward_fn_math as deepscaler_reward_fn
...
def _select_rm_score_fn(data_source):
    if data_source == 'openai/gsm8k':
        ...
        def _deepscaler_score(solution_str, ground_truth, **kwargs):
            result = deepscaler_reward_fn(
                data_source=data_source,
                llm_solution=solution_str,
                ground_truth=ground_truth,
            )
            return result.reward
        return _deepscaler_score
...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine, format_reward=False) -> None:
        ...
        self.format_reward = format_reward  # whether to use format reward
    ...
    def __call__(self, data: DataProto):
        ...
        def process_item(args):
            ...
            score = compute_score_fn(solution_str=sequences_str, ground_truth=ground_truth, format_reward=self.format_reward)
...
def main_task(config):
    ...
    # Only load ref policy when KL loss is actually used (kl_loss_coef > 0)
    ...
    reward_fn = RewardManager(tokenizer=tokenizer, num_examine=0, format_reward=config.reward_model.format_reward)

    # Note that we always use function-based RM for validation
    val_reward_fn = RewardManager(tokenizer=tokenizer, num_examine=1, format_reward=config.reward_model.format_reward)
