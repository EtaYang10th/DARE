...
def entropy_from_logits(logits: torch.Tensor):
    ...
    Memory-efficient: processes in chunks along dim 0 when the tensor
    would otherwise require a very large temporary allocation.
    Peak memory per chunk ≈ 3× chunk_bytes (softmax + logsumexp + pd*logits).
    """
    elem_bytes = 2 if logits.dtype in (torch.float16, torch.bfloat16) else 4
    tensor_bytes = logits.numel() * elem_bytes
    # 1 GiB threshold — peak usage is ~3× this, so ~3 GiB per chunk max
    CHUNK_BUDGET = 1 * (1024 ** 3)
    if tensor_bytes < CHUNK_BUDGET:
        pd = torch.nn.functional.softmax(logits, dim=-1)
        entropy = torch.logsumexp(logits, dim=-1) - torch.sum(pd * logits, dim=-1)
        return entropy
    # Chunked path: iterate over dim-0 to cap peak memory
    row_bytes = logits[0:1].numel() * elem_bytes
    chunk_size = max(1, CHUNK_BUDGET // row_bytes)
    entropy_chunks = []
    for i in range(0, logits.size(0), chunk_size):
        chunk = logits[i:i + chunk_size]
        pd = torch.nn.functional.softmax(chunk, dim=-1)
        ent = torch.logsumexp(chunk, dim=-1) - torch.sum(pd * chunk, dim=-1)
